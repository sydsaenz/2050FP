...
module number_sprite #(
    ...
    always_ff @(posedge pixel_clk) begin
        if (next) begin
            ...
            pixel_color <= (number[number_y] & (5'd1 << (5'd4-number_x))) > 0 ? 8'hff : 0;
        end else begin
            next <= 1;
            // size of pixel is what I divide by, 8x8 so divide by 8
            number_x <= (h_count - x) >> 5'd3;
            number_y <= (v_count - y) >> 5'd3;
        end
    end
